// Originally From: https://github.com/haripo/react-github-contribution-calendar/blob/master/src/index.tsx
...
interface Props {
  ...
  onPanelClick?: (date: string, value: number) => void;
}
...
// Used for screenshots of the app
const GENERATE_FAKE_DATA = false;

export default class EntryCalendar extends React.Component<Props, State> {
  ...
  makeCalendarData(history: { [k: string]: number }, lastDay: string, columns: number) {
    ...
    var result: ({ date: string; value: number; month: number } | null)[][] = [];
    for (var i = 0; i < columns; i++) {
      result[i] = [];
      for (var j = 0; j < 7; j++) {
        var date = lastWeekend.subtract((columns - i - 1) * 7 + (6 - j), 'day');
        if (date <= endDate) {
          const formattedDate = date.format(this.props.dateFormat);
          result[i][j] = {
            date: formattedDate,
            value: !GENERATE_FAKE_DATA
              ? history[formattedDate] || 0
              : Math.random() <= 0.75
              ? 1
              : 0,
            month: date.month(),
          ...
          <rect
            ...
            onClick={
              this.props.onPanelClick
                ? () => this.props.onPanelClick(contribution.date, contribution.value)
                : undefined
            }
          />
        );
